(function () {
	...
	app.value('ChangesList', customChangesList);
	...
	app.factory('DocumentPagesFactory', function ($resource) {
		return $resource(apiURL + '/api/GroupDocsComparison/DocumentPages?file=' + fileName + '&folderName=' + folderName + '&file2=' + fileName2 + '&userEmail=none&pageCount=0&lstPages=null', {}, {
			query: {
				method: 'GET',
				isArray: false
			}
	...
	app.factory('myService', function ($http) {
		var getData = function (page) {
			//console.log(apiURL + '/api/GroupDocsComparison/DocumentPages?file=' + fileName + '&folderName=' + folderName + '&file2=' + fileName2 + '&userEmail=none&currentPage=' + page);
			return $http({ method: "GET", url: apiURL + '/api/GroupDocsComparison/DocumentPages?file=' + fileName + '&folderName=' + folderName + '&file2=' + fileName2 + '&userEmail=none&currentPage=' + page }).then(function (result) {
				return result;
			}, function (error) {
		...
		if (page <= totalPages) {
			...
			myDataPromise.then(function (result) {
				...
				angular.forEach(result.data, function (value) {
					//console.log('value:  ' + value.TypeText);
					if (value.TypeText.indexOf('count') >= 0) {
						totalPages = parseInt(value.ActionText);
						UpdatePager();
						i++;
					}
					else if (value.TypeText.indexOf('page') >= 0) {
						if (value.ActionText.indexOf('C:') >= 0) {
							if (!customPageList.indexOf(value.ActionText) >= 0)
								customPageList.push(value.ActionText);
						...
						if (!customChangesList.indexOf(value) >= 0)
							customChangesList.push(value);
					}
					$scope.loading.hide();
				});
			});
	...
	app.controller('ComparisonAPIController',
		function ComparisonAPIController($scope, $sce, $http, $window, DocumentPagesFactory, myService, $loading, $timeout, $q, $alert) {
			var $that = this;
			...
			$scope.existApp = function () {
				window.location = '/comparison/total';
			};
			...
			if (customChangesList.length <= 0) {
				$scope.ChangesList = customChangesList;
			}
			...
			$scope.createPageImage = function (selectedFile, indx) {
				if (indx <= (imagedata.length - 1)) {
					...
					prevoiusIndx = indx;
					var imgData = $sce.trustAsResourceUrl(apiURL + '/api/GroupDocsComparison/pageimage?imagePath=' + selectedFile);
